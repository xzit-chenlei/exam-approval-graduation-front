  ...
  <div>
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <!-- 已选择人员展示（仅多选时显示） -->
        <div v-if="checkType === 'multiple' && selectedUserRows.length" class="selected-users">
          <span class="selected-label">已选择人员：</span>
          <el-tag
            v-for="user in selectedUserRows"
            :key="user.userId"
            size="mini"
...
<script>
import { listUserExcludeAdmin as listUser, deptTreeSelect } from "@/api/system/user";
...
export default {
  ...
  data() {
    return {
      ...
      selectUserList: [], // 回显数据传值（逗号分隔的 userId 字符串）
      selectedUserRows: [] // 已选择的完整用户行数据，用于展示
    };
  },
  watch: {
    // 根据名称筛选部门树
    ...
    userList: {
      handler(newVal) {
        if (StrUtil.isNotBlank(newVal) && this.selectUserList.length > 0) {
          const selectedIds = this.selectUserList.split(',');
          this.$nextTick(() => {
            if (!this.$refs.dataTable) return;
            this.$refs.dataTable.clearSelection();
            this.selectedUserRows = [];
            newVal.forEach(row => {
              if (selectedIds.includes(String(row.userId))) {
                this.$refs.dataTable.toggleRowSelection(row, true);
                this.selectedUserRows.push(row);
              }
            });
          });
        }
  ...
  methods: {
    ...
    // 这里必须用 userId，翻页时 ElementUI 才能保留已勾选的用户
    getRowKey (row) {
      return row.userId
    },
    // 筛选节点
