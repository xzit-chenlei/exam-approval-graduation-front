<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" fit :data="definitionList" border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="工单模板" align="center" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <el-button v-if="scope.row.formId" type="text" @click="handleForm(scope.row.formId)">
            <span>{{ scope.row.formName }}</span>
          </el-button>
          <label v-else>暂无工单</label>
        </template>
      </el-table-column>
      <el-table-column label="流程版本" align="center">
        <template slot-scope="scope">
          <el-tag size="medium" @click="handleVersion(scope.row)" style="cursor: pointer">v{{ scope.row.version }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="250" fixed="right" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button @click="handleAddForm(scope.row)" icon="el-icon-edit-el-icon-s-promotion" type="text" size="small"
                     v-if="scope.row.formId == null">配置主工单
          </el-button>
          <el-button
            @click="handleAddExamForm(scope.row)"
            icon="el-icon-edit"
            type="text"
            size="small"
          >
            {{ scope.row.examFormId == null ? '配置自定义工单' : '查看/修改自定义工单' }}
          </el-button>
          ...
          <el-button @click="handleWordTemplate(scope.row)" icon="el-icon-document" type="text" size="small">模板管理</el-button>
    ...
    <!--挂载考试表单-->
    <el-dialog :title="examFormDeployTitle" :visible.sync="examFormDeployOpen" width="60%" append-to-body>
      <div style="margin-bottom: 10px;">
        <span v-if="currentExamFormId">
          当前已绑定自定义工单：{{ currentExamFormName || ('ID: ' + currentExamFormId) }}
        </span>
        <span v-else>
          当前尚未绑定自定义工单
        </span>
      ...
      <el-row :gutter="24">
        <el-col :span="10" :xs="24">
          <el-table
              ...
              border
              highlight-current-row
              @current-change="handleExamFormCurrentChange"
              style="width: 100%">
            <el-table-column label="表单编号" align="center" prop="id"/>
            <el-table-column label="表单名称" align="center" prop="name"/>
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button size="mini" type="text" @click="submitExamFormDeploy(scope.row)">确定</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <pagination
              small
              layout="prev, pager, next"
              v-show="examFormTotal>0"
              :total="examFormTotal"
              :page.sync="examFormQueryParams.pageNum"
              :limit.sync="examFormQueryParams.pageSize"
              @pagination="ListExamFormDeploy"
          />
        </el-col>
        <el-col :span="14" :xs="24">
          <div class="test-form">
            <p>
              左侧选择一个自定义工单并点击“确定”进行绑定。
              每个流程仅支持绑定一个自定义工单，重新绑定将覆盖原有绑定。
            </p>
          </div>
        </el-col>
      </el-row>
    ...
    <!--历史版本-->
    <el-dialog title="历史版本" :visible.sync="showVersionQuery" width="60%" append-to-body>
      ...
      <el-table v-loading="loading" fit :data="versionDefinitionList" @selection-change="handleSelectionChange" border>
        <el-table-column type="selection" width="55" align="center"/>
        <el-table-column label="流程标识" align="center" prop="flowKey" :show-overflow-tooltip="true"/>
        <el-table-column label="流程分类" align="center" prop="category"/>
        <el-table-column label="流程名称" align="center" width="120" :show-overflow-tooltip="true">
...
import {listForm as listExamForm, addExamDeployForm, getFormByDeploy} from "@/api/exam/formModule";
...
export default {
  ...
  data() {
    return {
      ...
      // 当前流程已绑定的自定义工单信息（用于弹窗回显）
      currentExamFormId: null,
      currentExamFormName: '',
      formData: {}, // 默认表单数据
      readImage: {
        open: false,
  ...
  methods: {
    ...
    /** 跳转模板主表 */
    handleWordTemplate(row) {
      this.$router.push({
        path: '/exam/exam/wordTemplateNew/index',
        query: {
          flowableKey: row.flowKey
        }
      });
    },
    getVersionList() {
      this.loading = true;
      listDefinition(this.versionQueryParams).then(response => {
    ...
    handleVersion(row) {
      this.showVersionQuery = true;
      this.versionQueryParams.name = row.flowKey;
      this.getVersionList();

      // this.versionQueryName = row.name;
      // this.queryParams.derivedFrom = row.deploymentId;
      // this.getList();
    },
    /** 启动流程 */
    ...
    ListFormDeploy() {
      listForm(this.formQueryParams).then(res => {
        ...
        this.formDeployTitle = "挂载工单";
      })
    },
    /** 挂载考试表单弹框 */
    handleAddExamForm(row) {
      this.examFormDeployParam.deployId = row.deploymentId;
      // 先重置当前绑定信息
      this.currentExamFormId = null;
      this.currentExamFormName = '';
      // 查询当前流程已挂载的自定义工单
      getFormByDeploy(row.deploymentId).then(res => {
        const data = res.data || {};
        this.currentExamFormId = data.id || null;
        // 后端返回 Map 中通常包含 name/remark，这里优先使用 name，没有则退回 remark
        this.currentExamFormName = data.name || data.remark || '';
      }).catch(() => {
        // 未挂载或接口异常时，按未绑定处理
        this.currentExamFormId = null;
        this.currentExamFormName = '';
      }).finally(() => {
        this.ListExamFormDeploy();
      });
    },
    /** 挂载考试表单列表 */
    ListExamFormDeploy() {
      listExamForm(this.examFormQueryParams).then(res => {
        this.examFormList = res.rows;
        this.examFormTotal = res.total;
        this.examFormDeployOpen = true;
        this.examFormDeployTitle = "挂载自定义工单";
        this.$nextTick(() => {
          if (this.currentExamFormId && this.$refs.examSingleTable) {
            const current = this.examFormList.find(item => item.id === this.currentExamFormId);
            if (current) {
              this.$refs.examSingleTable.setCurrentRow(current);
            }
          }
        });
      })
    ...
    /** 挂载考试表单 */
    submitExamFormDeploy(row) {
      const doBind = () => {
        this.examFormDeployParam.formId = row.id;
        addExamDeployForm(this.examFormDeployParam).then(res => {
          this.$modal.msgSuccess(res.msg);
          this.examFormDeployOpen = false;
          this.getList();
        })
      }

      // 已有绑定且本次选择的是不同自定义工单时，提示覆盖
      if (this.currentExamFormId && this.currentExamFormId !== row.id) {
        this.$modal.confirm('该流程已绑定其他自定义工单，确定要覆盖吗？').then(() => {
          doBind();
        });
      } else {
        doBind();
    ...
    handleExamFormCurrentChange(data) {
      // 考试表单选择变化时的处理，可以根据需要添加
    },
    // /** 更改挂载表单弹框 */
    // handleEditForm(row){
    //   this.formDeployParam.deployId = row.deploymentId
