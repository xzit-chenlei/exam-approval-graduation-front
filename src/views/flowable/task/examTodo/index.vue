<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="todoList" border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="教研室" align="center" prop="officeName"/>
  ...
  methods: {
    ...
    getList() {
      ...
      todoList(this.queryParams).then(response => {
        ...
        // 前端兜底：只显示自定义表单流程
        this.todoList = records.filter(r => r.formMode === 'custom' || r.examFormId || r.examFormData);
    ...
    // 跳转到处理页面（自定义表单版本）
    handleProcess(row){
      this.$router.push({ path: '/flowable/task/examTodo/detail/index',
        query: {
